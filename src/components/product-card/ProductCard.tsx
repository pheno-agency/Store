...
const ProductCard = component$<ProductProps>(({ cover, name, description, price }) => {
  return (
    <div class="flex flex-col items-center w-170px h-260px border rounded-8px p-10px bg-white">
      {!cover ? <Image src={coverImg} width={30}
        height={30} alt={name} class="w-100% h-100px mx-auto" /> :
        <Image src={cover} width={30}
          height={30} alt={name} class="w-100% h-100px mx-auto" />}
